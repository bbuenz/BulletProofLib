...
public class CircuitVerifier<T extends GroupElement<T>> implements Verifier<GeneratorParams<T>, ArithmeticCircuit<T>, CircuitProof<T>> {
    ...
    @Override
    public void verify(GeneratorParams<T> parameter, ArithmeticCircuit<T> circuit, CircuitProof<T> proof) throws VerificationFailedException {
        ...
        BigInteger z = ProofUtils.hash("z", y).mod(q);
