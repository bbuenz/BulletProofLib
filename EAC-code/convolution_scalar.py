...
def _inverse_butterfly(L1, L2, r):
   ...
   assert 0 <= r <= len(L1)
   K = len(L1)
   #L1=[LPoly.Ve(x) for x in L1]
   #L2=[LPoly.Ve(x) for x in L2]
...
def _split(L, m, k):
   ...
   zeroes = [zero] * K
   #print len(zeroes)
   #for i in xrange(len(zeroes)):zeroes[i]=LPoly.Ve(zeroes[i])
   #print L,m,k
