...
def _inverse_butterfly(L1, L2, r):
   ...
   assert 0 <= r <= len(L1)
   K = len(L1)
   L1=[np.array(x) for x in L1]#L1=[LPoly.Ve(x) for x in L1]
   L2=[np.array(x) for x in L2]#L2=[LPoly.Ve(x) for x in L2]
...
def _split(L, m, k):
   ...
   zeroes = [zero] * K
   #print len(zeroes)
   for i in xrange(len(zeroes)):zeroes[i]=np.array(zeroes[i])#=LPoly.Ve(zeroes[i])
   #print L,m,k
...
"""
   ...
   #return [x / M for x in L3]

   return [[y // M for y in x] for x in L3]
...
def _convolution_fft(L1, L2):
   ...
   #zero = R(0)
   zero=np.array([0 for i in L1[0]])#zero=LPoly.Ve([0 for i in L1[0]])
   #zero=0
   #L1 = L1 + [zero] * (N - len(L1))
   #L2 = L2 + [zero] * (N - len(L2))

   L1.extend([zero for i in xrange(N - len(L1))])
   L2.extend([zero for i in xrange(N - len(L2))])
   ...
   #print [L3[i] / M for i in range(outlen)]
   #return [L3[i] / M for i in range(outlen)]
   return [[x // M for x in L3[i]] for i in range(outlen)]
